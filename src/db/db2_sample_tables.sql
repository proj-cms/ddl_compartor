-- Tables-only script for DB2: creates tables in test_schema if they don't exist
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_cnt FROM all_tables WHERE owner = 'test_schema' AND table_name = 'ONLY_IN_DB2';
    IF v_cnt = 0 THEN
        EXECUTE IMMEDIATE q'[CREATE TABLE test_schema.only_in_db2 (
            id NUMBER(10) PRIMARY KEY,
            description VARCHAR2(100)
        )]';
        DBMS_OUTPUT.PUT_LINE('Created table test_schema.ONLY_IN_DB2');
    ELSE
        DBMS_OUTPUT.PUT_LINE('test_schema.ONLY_IN_DB2 already exists');
    END IF;
